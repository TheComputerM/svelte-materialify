<script>
  ...
  export let format = (val) => (Array.isArray(val) ? (val.map((vl) =>items.find((v)=>v.value==vl) ? items.find((v)=>v.value==vl).name : "no items").join(', ')) : (items.find((v)=>v.value == val) ? items.find((v)=>v.value == val).name : "no items" ));
